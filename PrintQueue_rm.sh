...
if [ "$current_path" = "/app/jdedwardsppack/e920/PrintQueue" ];
   then
        ...
        find . -mtime +178 -name "*_PDF" > /tmp/print_queue_pdf.txt
        find . -mtime +178 -name "*.csv" > /tmp/print_queue_csv.txt
        find . -mtime +178 -name "*_PDF" -exec rm {} \;
        find . -mtime +178 -name "*.csv" -exec rm {} \;
    else
